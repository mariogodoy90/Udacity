...
def column_to_list(data, index):
    """
    Cria uma nova lista com a coluna informada pelo index.
        Argumentos:
            data: Lista retirada do arquivo principal (csv)
            index: Coluna que deseja filtrar (numerico)
        Retorna:
            Lista contendo os valores da coluna seleciona pela index.
    """
    column_list = []
...
def count_gender(data_list):
    """
    Retorna a contagem dos generos (Masculino/Feminino)
        Argumentos:
            data: Lista retirada do arquivo principal (csv)
        Retorna:
            Lista com a soma do total de [Masculino, Feminino]
    """
    male = 0
    female = 0
...
def most_popular_gender(data):
    """
    Retorna o genero de maior popularidade
        Argumentos:
            data: Lista retirada do arquivo principal (csv)
        Retorna:
            Genero de maior popularidade
    """
    answer = ""
...
def count_types(data):
    """
    Retorna o total de cada um dos tipos de usuarios
        Argumentos:
            data: lista com os tipos de usuarios
        Retorna:
            Total dos tipos de usuarios [susbcriber, dependent, customer]
    """
    subscriber, dependent, customer = 0, 0, 0
    subscriber = user_types_list.count('Subscriber')
    dependent = user_types_list.count('Dependent')
    customer = user_types_list.count('Customer')
    return [subscriber, dependent, customer]

user_types_list = column_to_list(data_list, -3)
types = list(set(user_types_list))
quantity = count_types(user_types_list)
y_pos = list(range(len(types)))
plt.bar(y_pos, quantity)
plt.ylabel('Quantidade')
...
answer = "Existem registros sem a informação do genero"
...
def count_items(column_list):
    """
    Retorna a soma da categoria solicitada
        Argumentos:
            data: Lista filtrada da categoria desejada
        Retorna:
            item_types: Categorias da coluna seleciona
            count_items: Total de cada categoria
    """
    item_types = []
    count_items = []
    contagem = {}
    for dado in column_list:
        if dado in contagem:
            contagem[dado] +=1
        else:
            contagem[dado] = 1

    item_types = list(contagem.keys())
    count_items = list(contagem.values())
